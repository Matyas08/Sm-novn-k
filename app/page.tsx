...
type ShiftType = "morning" | "afternoon" | "night" | "all_day" | "vacation" | "sick";
...
export default function Home() {
  ...
  const stats = useMemo(() => people.map(person => {
    ...
    return { person, shifts: ps.length, hours: Math.round(hours), morning: ps.filter(s=>s.type==="morning").length, afternoon: ps.filter(s=>s.type==="afternoon").length, night: ps.filter(s=>s.type==="night").length, vacation: ps.filter(s=>s.type==="vacation").length, sick: ps.filter(s=>s.type==="sick").length };
  }), [people, shifts]);
...
function StatBox({label,value}:{label:string;value:number}){return <div className="rounded-2xl border border-white/[0.05] bg-white/[0.025] p-4"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 text-2xl font-black text-white">{value}</div></div>}
