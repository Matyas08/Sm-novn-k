...
const shiftInfo: Record<ShiftType, { label: string; short: string; color: string; icon: string }> = {
  ...
  midnight: { label: "Půlnoční směna", short: "Půlnoční", color: "#f472b6", icon: "moon" },
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setSelectedPerson(prev => prev ?? mapped.find(p => p.email.toLowerCase() === signedEmail) ?? null);
    })();
  }, [session]);
  ...
  const stats = useMemo(() => people.map(person => {
    ...
    const hours = ps.reduce((sum,s)=>{ const [sh,sm]=s.startTime.split(":").map(Number), [eh,em]=s.endTime.split(":").map(Number); let a=sh*60+sm,b=eh*60+em; if(b<=a)b+=1440; return sum+(b-a)/60; },0);
    return {
      person,
      shifts: ps.length,
...
function ShiftsPage({people,currentPerson,selectedPerson,setSelectedPerson,month,year,changeMonth,filter,setFilter,openAddShift,openEditShift,getShift,loading}:{people:Person[];shifts:Shift[];currentPerson:Person|null;selectedPerson:Person|null;setSelectedPerson:(p:Person|null)=>void;month:number;year:number;changeMonth:(d:number)=>void;filter:ShiftType|"all";setFilter:(f:ShiftType|"all")=>void;openAddShift:(d?:string)=>void;openEditShift:(s:Shift)=>void;getShift:(pid:string,d:string)=>Shift|undefined;loading:boolean;}){
...
}}><div className="truncate">{p.name}</div><div className="opacity-70">{s.startTime}–{s.endTime}</div></div>})}</div></div>})}</div></Card><div className="mt-4 flex items-center gap-2 rounded-2xl border border-white/[0.05] bg-white/[0.02] px-4 py-3 text-xs text-slate-500"><Icon name="lock" size={15}/>{loading?"Načítám směny…":"Upravovat můžeš pouze svoje směny. Směny ostatních jsou pouze k zobrazení."}</div></div>;
}
...
function StatsPage({stats,selectedId,setSelectedId}:{stats:{person:Person;shifts:number;hours:number;morning:number;afternoon:number;intershift:number;night:number;midnight:number;all_day:number;vacation:number;sick:number}[];selectedId:string|null;setSelectedId:(id:string)=>void}){
  ...
  return <div>
    ...
    <div className="grid gap-5 xl:grid-cols-[300px_1fr]">
      <Card className="p-4"><div className="space-y-2">{stats.map(s=><button key={s.person.id} onClick={()=>setSelectedId(s.person.id)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${selected?.person.id===s.person.id?"bg-white/[0.07]":"hover:bg-white/[0.03]"}`}><Avatar person={s.person} size={38}/><div><div className="text-sm font-semibold">{s.person.name}</div><div className="text-xs text-slate-500">{s.shifts} směn · {s.hours} h</div></div></button>)}</div></Card>
      <div className="space-y-5">
        {selected&&<Card className="p-6"><div className="flex items-center gap-4"><Avatar person={selected.person} size={54}/><div><h2 className="text-xl font-bold">{selected.person.name}</h2><p className="text-sm text-slate-500">Osobní statistiky</p></div></div><div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4"><StatBox label="Směny" value={selected.shifts}/><StatBox label="Hodiny" value={selected.hours}/><StatBox label="Ranní" value={selected.morning} color={shiftInfo.morning.color}/><StatBox label="Odpolední" value={selected.afternoon} color={shiftInfo.afternoon.color}/><StatBox label="Mezisměna" value={selected.intershift} color={shiftInfo.intershift.color}/><StatBox label="Noční" value={selected.night} color={shiftInfo.night.color}/><StatBox label="Půlnoční" value={selected.midnight} color={shiftInfo.midnight.color}/><StatBox label="Celodenní" value={selected.all_day} color={shiftInfo.all_day.color}/><StatBox label="Dovolená" value={selected.vacation} color={shiftInfo.vacation.color}/><StatBox label="Nemoc" value={selected.sick} color={shiftInfo.sick.color}/></div></Card>}
        <Card className="p-6"><h2 className="font-bold">Porovnání všech</h2><div className="mt-5 space-y-3">{stats.map(s=><div key={s.person.id} className="flex items-center gap-3"><Avatar person={s.person} size={32}/><div className="w-24 text-sm font-semibold">{s.person.name}</div><div className="h-2 flex-1 overflow-hidden rounded-full bg-white/[0.05]"><div className="h-full rounded-full" style={{width:`${Math.min(100,(s.hours/Math.max(1,...stats.map(x=>x.hours)))*100)}%`,background:`linear-gradient(90deg,${s.person.color},${s.person.color}99)`}}/></div><div className="w-16 text-right text-xs text-slate-500">{s.hours} h</div></div>)}</div></Card>
